# Asset Location Walk.test

> 12 nodes · cohesion 0.23

## Key Concepts

- **asset-location-walk.test.js** (10 connections) — `test/asset-location-walk.test.js`
- **asset-location-walk.js** (4 connections) — `src/asset-location-walk.js`
- **resolveAssetLocationChain()** (4 connections) — `src/asset-location-walk.js`
- **isPlaceholderName()** (3 connections) — `src/asset-location-walk.js`
- **getCharacterAssets()** (2 connections) — `src/character_info_db.js`
- **ctx()** (2 connections) — `test/asset-location-walk.test.js`
- **nested()** (2 connections) — `test/asset-location-walk.test.js`
- **resolved()** (2 connections) — `test/asset-location-walk.test.js`
- **assert** (1 connections) — `test/asset-location-walk.test.js`
- **LOC_FIELDS** (1 connections) — `test/asset-location-walk.test.js`
- **{ resolveAssetLocationChain, isPlaceholderName }** (1 connections) — `test/asset-location-walk.test.js`
- **{ test }** (1 connections) — `test/asset-location-walk.test.js`

## Relationships

- [Character Database](Character_Database.md) (3 shared connections)

## Source Files

- `src/asset-location-walk.js`
- `src/character_info_db.js`
- `test/asset-location-walk.test.js`

## Audit Trail

- EXTRACTED: 26 (79%)
- INFERRED: 7 (21%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
# Fitting Simulator (_fitEffCargo)

> 14 nodes · cohesion 0.25

## Key Concepts

- **_fitEsc()** (23 connections) — `src/func/fitting.js`
- **_fitRenderBays()** (19 connections) — `src/func/fitting.js`
- **_fitAddCargo()** (8 connections) — `src/func/fitting.js`
- **_fitImportFromGame()** (8 connections) — `src/func/fitting.js`
- **_fitCargoHtml()** (6 connections) — `src/func/fitting.js`
- **_fitPickList()** (6 connections) — `src/func/fitting.js`
- **_fitEffCargo()** (5 connections) — `src/func/fitting.js`
- **_fitCargoUsedM3()** (4 connections) — `src/func/fitting.js`
- **_fitImplantPanelHtml()** (4 connections) — `src/func/fitting.js`
- **_fitCargoSetQty()** (3 connections) — `src/func/fitting.js`
- **_fitImplantCount()** (3 connections) — `src/func/fitting.js`
- **_fitPickOpen()** (3 connections) — `src/func/fitting.js`
- **_fitPickMessage()** (3 connections) — `src/func/fitting.js`
- **_fitPickMatches()** (2 connections) — `src/func/fitting.js`

## Relationships

- [Fitting Simulator](Fitting_Simulator.md) (15 shared connections)
- [Fitting Slots & Module Placement](Fitting_Slots_%26_Module_Placement.md) (13 shared connections)
- [Fitting Drones & Stats Display](Fitting_Drones_%26_Stats_Display.md) (12 shared connections)
- [Fitting Simulator (_fitPersist)](Fitting_Simulator_%28_fitPersist%29.md) (5 shared connections)
- [Fitting Fighters & Canvas Rings](Fitting_Fighters_%26_Canvas_Rings.md) (4 shared connections)
- [Fitting Skills & Trait Engine](Fitting_Skills_%26_Trait_Engine.md) (1 shared connections)
- [Gridstack Touch & Drag-Drop](Gridstack_Touch_%26_Drag-Drop.md) (1 shared connections)

## Source Files

- `src/func/fitting.js`

## Audit Trail

- EXTRACTED: 95 (98%)
- INFERRED: 2 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
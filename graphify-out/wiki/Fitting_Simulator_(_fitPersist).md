# Fitting Simulator (_fitPersist)

> 20 nodes · cohesion 0.16

## Key Concepts

- **_fitRenderBrowser()** (12 connections) — `src/func/fitting.js`
- **_fitRenderTree()** (8 connections) — `src/func/fitting.js`
- **_fitSaveLocal()** (8 connections) — `src/func/fitting.js`
- **_fitDoSearch()** (7 connections) — `src/func/fitting.js`
- **_fitFillTypes()** (7 connections) — `src/func/fitting.js`
- **_fitPassesFilters()** (7 connections) — `src/func/fitting.js`
- **_fitHideGameFit()** (7 connections) — `src/func/fitting.js`
- **_fitLocalFits()** (6 connections) — `src/func/fitting.js`
- **_fitRestoreHiddenGameFits()** (6 connections) — `src/func/fitting.js`
- **_fitDeleteLocalFit()** (6 connections) — `src/func/fitting.js`
- **_fitEnsureGameFits()** (5 connections) — `src/func/fitting.js`
- **_fitComputeUsage()** (5 connections) — `src/func/fitting.js`
- **_fitTypeRowHtml()** (4 connections) — `src/func/fitting.js`
- **_fitHiddenGameFits()** (4 connections) — `src/func/fitting.js`
- **_fitPersist()** (3 connections) — `src/func/fitting.js`
- **_fitFilled()** (3 connections) — `src/func/fitting.js`
- **_fitLocalFitsStore()** (3 connections) — `src/func/fitting.js`
- **_fitHiddenGameFitsStore()** (3 connections) — `src/func/fitting.js`
- **_fitHiddenKey()** (3 connections) — `src/func/fitting.js`
- **_fitHullNames()** (2 connections) — `src/func/fitting.js`

## Relationships

- [Fitting Simulator](Fitting_Simulator.md) (20 shared connections)
- [Fitting Slots & Module Placement](Fitting_Slots_%26_Module_Placement.md) (16 shared connections)
- [Fitting Simulator (_fitEffCargo)](Fitting_Simulator_%28_fitEffCargo%29.md) (5 shared connections)
- [Fitting Drones & Stats Display](Fitting_Drones_%26_Stats_Display.md) (2 shared connections)
- [Fitting Skills & Trait Engine](Fitting_Skills_%26_Trait_Engine.md) (2 shared connections)
- [Theme Vars](Theme_Vars.md) (1 shared connections)
- [Fitting Fighters & Canvas Rings](Fitting_Fighters_%26_Canvas_Rings.md) (1 shared connections)

## Source Files

- `src/func/fitting.js`

## Audit Trail

- EXTRACTED: 102 (94%)
- INFERRED: 7 (6%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
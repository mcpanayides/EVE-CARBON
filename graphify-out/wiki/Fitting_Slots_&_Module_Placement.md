# Fitting Slots & Module Placement

> 28 nodes · cohesion 0.20

## Key Concepts

- **_fitRenderAll()** (34 connections) — `src/func/fitting.js`
- **_fitFlash()** (33 connections) — `src/func/fitting.js`
- **_fitHandleDrop()** (12 connections) — `src/func/fitting.js`
- **_fitAddDrone()** (11 connections) — `src/func/fitting.js`
- **_fitLoadGameFit()** (11 connections) — `src/func/fitting.js`
- **_fitImportEFT()** (10 connections) — `src/func/fitting.js`
- **_fitApplySnapshot()** (8 connections) — `src/func/fitting.js`
- **_fitAddModule()** (8 connections) — `src/func/fitting.js`
- **_fitMod()** (8 connections) — `src/func/fitting.js`
- **_fitLoadHull()** (7 connections) — `src/func/fitting.js`
- **_fitCanFit()** (7 connections) — `src/func/fitting.js`
- **_fitPlaceSubsystem()** (7 connections) — `src/func/fitting.js`
- **_fitCloneSlot()** (7 connections) — `src/func/fitting.js`
- **_fitLoadChargeAll()** (7 connections) — `src/func/fitting.js`
- **_fitPickResult()** (6 connections) — `src/func/fitting.js`
- **_fitEffSlots()** (6 connections) — `src/func/fitting.js`
- **_fitTubeUnits()** (6 connections) — `src/func/fitting.js`
- **_fitLoadCharge()** (6 connections) — `src/func/fitting.js`
- **_fitShowTubeMenu()** (6 connections) — `src/func/fitting.js`
- **_fitSyncRacks()** (5 connections) — `src/func/fitting.js`
- **_fitPlace()** (5 connections) — `src/func/fitting.js`
- **_fitCloneWear()** (5 connections) — `src/func/fitting.js`
- **_fitLoadLocalFit()** (5 connections) — `src/func/fitting.js`
- **_fitEmptyRacks()** (4 connections) — `src/func/fitting.js`
- **_fitDupMod()** (4 connections) — `src/func/fitting.js`
- *... and 3 more nodes in this community*

## Relationships

- [Fitting Simulator](Fitting_Simulator.md) (39 shared connections)
- [Fitting Simulator (renderFitting)](Fitting_Simulator_%28renderFitting%29.md) (23 shared connections)
- [Fitting Fighters & Canvas Rings](Fitting_Fighters_%26_Canvas_Rings.md) (12 shared connections)
- [Fitting Simulator (_fitPassesFilters)](Fitting_Simulator_%28_fitPassesFilters%29.md) (10 shared connections)
- [Fitting Drones & Stats Display](Fitting_Drones_%26_Stats_Display.md) (7 shared connections)
- [Market Trading](Market_Trading.md) (1 shared connections)

## Source Files

- `src/func/fitting.js`

## Audit Trail

- EXTRACTED: 239 (100%)
- INFERRED: 1 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
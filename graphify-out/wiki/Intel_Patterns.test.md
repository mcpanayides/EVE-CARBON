# Intel Patterns.test

> 26 nodes · cohesion 0.13

## Key Concepts

- **intel_patterns.test.js** (29 connections) — `test/intel_patterns.test.js`
- **patterns.js** (15 connections) — `src/intel/patterns.js`
- **countOccasions()** (5 connections) — `src/intel/patterns.js`
- **createPatternStore()** (5 connections) — `src/intel/patterns.js`
- **dayIndex()** (5 connections) — `src/intel/patterns.js`
- **corridors()** (4 connections) — `src/intel/patterns.js`
- **entryPoints()** (4 connections) — `src/intel/patterns.js`
- **routeStats()** (4 connections) — `src/intel/patterns.js`
- **bucketPattern()** (3 connections) — `src/intel/patterns.js`
- **DEFAULTS** (2 connections) — `src/intel/patterns.js`
- **WEEKDAY_NAMES** (2 connections) — `src/intel/patterns.js`
- **at()** (2 connections) — `test/intel_patterns.test.js`
- **seededHistory()** (2 connections) — `test/intel_patterns.test.js`
- **store()** (2 connections) — `test/intel_patterns.test.js`
- **hourOf()** (1 connections) — `src/intel/patterns.js`
- **weekdayOf()** (1 connections) — `src/intel/patterns.js`
- **assert** (1 connections) — `test/intel_patterns.test.js`
- **CHAN** (1 connections) — `test/intel_patterns.test.js`
- **{
  createPatternStore, bucketPattern, hourBlocks, buildWalks, routeStats, corridors,
  entryPoints, countOccasions, DEFAULTS, WEEKDAY_NAMES,
}** (1 connections) — `test/intel_patterns.test.js`
- **{ createProximityTracker, buildAdjacency }** (1 connections) — `test/intel_patterns.test.js`
- **evLine()** (1 connections) — `test/intel_patterns.test.js`
- **hop()** (1 connections) — `test/intel_patterns.test.js`
- **hourRow()** (1 connections) — `test/intel_patterns.test.js`
- **SDE** (1 connections) — `test/intel_patterns.test.js`
- **sysSighting()** (1 connections) — `test/intel_patterns.test.js`
- *... and 1 more nodes in this community*

## Relationships

- [Intel Service](Intel_Service.md) (3 shared connections)
- [Intel Channel Parser](Intel_Channel_Parser.md) (3 shared connections)
- [Fleet Mining](Fleet_Mining.md) (2 shared connections)
- [Fitting Tree & Scheduling](Fitting_Tree_%26_Scheduling.md) (2 shared connections)
- [Intel Resume.test](Intel_Resume.test.md) (2 shared connections)

## Source Files

- `src/intel/patterns.js`
- `test/intel_patterns.test.js`

## Audit Trail

- EXTRACTED: 82 (85%)
- INFERRED: 14 (15%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
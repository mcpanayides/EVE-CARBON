# s

> God node · 59 connections · `src/vendor/gridstack/gridstack-all.js`

**Community:** [Gridstack Geometry & Collision](Gridstack_Geometry_%26_Collision.md)

## Connections by Relation

### calls
- ._setupAcceptWidget() `EXTRACTED`
- .columnChanged() `EXTRACTED`
- .swap() `EXTRACTED`

### contains
- [_](_.md) `EXTRACTED`

### method
- .moveNode() `EXTRACTED`
- ._fixCollisions() `EXTRACTED`
- .addNode() `EXTRACTED`
- ._notify() `EXTRACTED`
- .nodeBoundFix() `EXTRACTED`
- ._packNodes() `EXTRACTED`
- .moveNodeCheck() `EXTRACTED`
- .sortNodes() `EXTRACTED`
- .batchUpdate() `EXTRACTED`
- .collide() `EXTRACTED`
- .findEmptyPosition() `EXTRACTED`
- .prepareNode() `EXTRACTED`
- .restoreInitial() `EXTRACTED`
- .willItFit() `EXTRACTED`
- .beginUpdate() `EXTRACTED`
- .compact() `EXTRACTED`
- .findCacheLayout() `EXTRACTED`
- .save() `EXTRACTED`
- .saveInitial() `EXTRACTED`
- .cacheOneLayout() `EXTRACTED`

### references
- .constructor() `EXTRACTED`
- .update() `EXTRACTED`
- ._dragOrResize() `EXTRACTED`
- .createDiv() `EXTRACTED`
- .getValuesFromTransformedElement() `EXTRACTED`
- .getCellHeight() `EXTRACTED`
- .defaults() `EXTRACTED`
- droppable() `EXTRACTED`
- .removePositioningStyles() `EXTRACTED`
- ._applyChange() `EXTRACTED`
- ._getChange() `EXTRACTED`
- ._getClipping() `EXTRACTED`
- ._getDragOffset() `EXTRACTED`
- .cloneNode() `EXTRACTED`
- .addElStyles() `EXTRACTED`
- .getElements() `EXTRACTED`
- .getElement() `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*
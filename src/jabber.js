...
// ── Filters (kept in sync with checkbox state) ────────────────────────────────
// jabberFilterDirectorOnly is declared in state.js — do not re-declare here.
// jabberFilterBroadcastOnly is local to jabber.js (not in state.js yet).
if (typeof jabberFilterBroadcastOnly === 'undefined') var jabberFilterBroadcastOnly = true;
...
function jabberApplyFilters(rows) {
  return rows.filter(row => {
    if (jabberFilterDirectorOnly  && !row.is_director)  return false;
    if (jabberFilterBroadcastOnly && !row.sig)           return false;
    return true;
  });
}
...
function renderJabberTable() {
  ...
  const filtered = jabberApplyFilters(sorted);

  if (!filtered.length) {
    const msg = jabberMessages.length
      ? 'No messages match the current filters.'
      : 'No messages received yet.';
    tbody.innerHTML = `<tr><td colspan="9" class="loading-row">${escHtml(msg)}</td></tr>`;
    updateJabberSummary(filtered.length);
    return;
  }
  ...
  tbody.innerHTML = filtered.map(row => {
    ...
    return `<tr title="${escHtml(row.raw_body || '')}">
      <td style="font-family:var(--mono); font-size:11px; white-space:nowrap;">${escHtml(row.eve_timecode || row.ping_timestamp || '')}</td>
      <td>${escHtml(row.fc_name       || '')}</td>
      <td>${escHtml(row.formup_location || '')}</td>
      <td>${escHtml(row.pap_type      || '')}</td>
...
function updateJabberSummary(count) {
  ...
  if (!summary) return;
  if (count === undefined) {
    const filtered = jabberApplyFilters(jabberMessages);
...
function bindJabberEvents() {
  // Director-only filter checkbox (in the page toolbar)
  const directorCb = document.getElementById('jabberDirectorOnly');
  if (directorCb) {
    directorCb.addEventListener('change', () => {
      jabberFilterDirectorOnly = directorCb.checked;
      renderJabberTable();
    });
  }

  // Broadcasts-only filter checkbox
  const broadcastCb = document.getElementById('jabberBroadcastOnly');
  if (broadcastCb) {
    broadcastCb.addEventListener('change', () => {
      jabberFilterBroadcastOnly = broadcastCb.checked;
      renderJabberTable();
    });
  }
  ...
  window.eveAPI.on('jabber-status', (status) => {
    ...
    // Load history once we know the page is live (also called on page open)
    if (status.status === 'online') {
      loadJabberHistory();
    }
  });

  // Load whatever is in the DB immediately (works even if already connected)
  loadJabberHistory();
